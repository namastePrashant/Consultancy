...
class Admin_MedicalController extends Zend_Controller_Action {
    ...
    public function addAction() {
        ...
        if ($id) {
            ...
            $candidate = $candidatemodel->getKeysAndValuesById($id);
            $exp = explode("::", $candidate);
            if (sizeof($exp) > 0) {
                $form->passport_no->setValue($exp[2]);
            }
            $form->candidate_id->setValue($candidate);
        }
        $this->view->form = $form;
    ...
    public function listAction() {
        ...
        $furtherProcessingColumn->setPosition('right')->setName('Processing')->setDecorator("<a href=\"$baseUrl/admin/medical/medical-test/id/{{candidate_id}}\">Processing</a><input class=\"address-id\" name=\"address_id[]\" type=\"hidden\" value=\"{{candidate_id}}\"/>");
        ...
        $grid->addExtraColumns($furtherProcessingColumn, $editColumn, $deleteColumn);
...
//            $data['add_company_id'] = $menu['add_company_id'];
    ...
    public function medicalTestAction() {
        $id = $this->_getParam('id', 0);
        $this->view->id = $id;
        if ($this->getRequest()->isPost()) {
            $medical = $this->_getParam('medical');
            ...
            if ('Yes' == $medical) {
                try {
                    ...
                    $this->_helper->redirector('add', "stamping", "admin", array('id' => $id));
                } catch (Exception $e) {
                    $this->_helper->FlashMessenger->addMessage(array("error" => "This Candidate has already taken the medical test."));
                    $this->_helper->redirector('list');
                }
            } elseif ('No' == $medical) {
                try {
                    ...
                    $medicalTestModel = new Admin_Model_MedicalTest();
                    $lastId = $medicalTestModel->add($formData);
                    $this->_helper->redirector('fail', "medical", "admin", array('id' => $id));
                } catch (Exception $e) {
                    $this->_helper->FlashMessenger->addMessage(array("error" => "This Candidate has already taken the medical test."));
                    $this->_helper->redirector('list');
                }
