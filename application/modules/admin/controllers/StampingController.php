...
class Admin_StampingController extends Zend_Controller_Action {
    ...
    public function addAction() {
        ...
        if ($id) {
            ...
            $option = $medicalmodel->getMedicallyApprovedById($id);
    ...
    public function stampingTestAction() {
        $id = $this->_getParam('id', 0);
        $this->view->id = $id;
        if ($this->getRequest()->isPost()) {
            $stamping = $this->_getParam('stamping');

            if ('Yes' == $stamping) {
                try {
                    $formData = array();
                    $formData['status'] = 'STP';
                    $medicalTestModel = new Admin_Model_MedicalTest();
                    $lastId = $medicalTestModel->update($formData, $id);
                    $this->_helper->redirector('shram-test', "stamping", "admin", array('id' => $id));
                } catch (Exception $e) {
                    $this->_helper->FlashMessenger->addMessage(array("error" => "This Candidate has already taken the Stamping  test."));
                    $this->_helper->redirector('list');
                }
            } elseif ('No' == $stamping) {
                try {
                    ...
                    $medicalTestModel = new Admin_Model_MedicalTest();
                    $lastId = $medicalTestModel->update($formData, $id);
                    $this->_helper->redirector('fail', "stamping", "admin", array('id' => $id));
                } catch (Exception $e) {
                    $this->_helper->FlashMessenger->addMessage(array("error" => "This Candidate has already taken the Stamping test."));
                    $this->_helper->redirector('list');
                }
    ...
    public function failAction() {
        ...
        $where = "STF";
        $medicalTestModel = new Admin_Model_MedicalTest();
        $data = $medicalTestModel->getAllFail($where);
        $this->view->result = $data;
    }

    public function shramTestAction() {
        $id = $this->_getParam('id', 0);
        $this->view->id = $id;
        if ($this->getRequest()->isPost()) {
            $shram = $this->_getParam('shram');
            ...
            if ('Yes' == $shram) {
                try {
                    ...
                    $formData['status'] = 'Y';
                    $medicalTestModel = new Admin_Model_ShramTest();
                    $lastId = $medicalTestModel->add($formData);
                    $this->_helper->redirector('orientation-test', "stamping", "admin", array('id' => $id));
                } catch (Exception $e) {
                    $this->_helper->FlashMessenger->addMessage(array("error" => "This Candidate has already taken the Shram test."));
                    $this->_helper->redirector('list');
                }
            } elseif ('No' == $shram) {
                try {
    ...
    public function shramFailAction() {
        $id = $this->_getParam('id', 0);
        $where = "N";
        $medicalTestModel = new Admin_Model_ShramTest();
        $data = $medicalTestModel->getAllFail($where);
    ...
    public function orientationTestAction() {
        $id = $this->_getParam('id', 0);
        $this->view->id = $id;
        if ($this->getRequest()->isPost()) {
            ...
            if ('Yes' == $orientation) {
                try {
                    $formData = array();
                    $formData['orientation_status'] = 'Y';
                    $medicalTestModel = new Admin_Model_ShramTest();
                    $lastId = $medicalTestModel->update($formData, $id);
                    $this->_helper->redirector('add', "ticketing", "admin", array('id' => $id));
                } catch (Exception $e) {
                    $this->_helper->FlashMessenger->addMessage(array("error" => "This Candidate has already taken the orientation test."));
                    $this->_helper->redirector('list');
                }
            } elseif ('No' == $orientation) {
                try {
                    $formData = array();
                    $formData['orientation_status'] = 'N';
                    $medicalTestModel = new Admin_Model_ShramTest();
                    $lastId = $medicalTestModel->update($formData, $id);
                    $this->_helper->redirector('add', "ticketing", "admin", array('id' => $id));
                } catch (Exception $e) {
