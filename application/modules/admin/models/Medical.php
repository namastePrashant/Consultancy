...
class Admin_Model_Medical {
    ...
    public function getMedicallyApproved() {
        $db = Zend_Db_Table::getDefaultAdapter();
        $select = $db->select();
        $select->from(array("t" => "consultancy_medical_test"))
                ->joinLeft(array("c" => "consultancy_candidate"), "t.candidate_id=c.candidate_id", array("c.candidate_id", "c.name", "c.passport_no"))
                ->joinLeft(array("m" => "consultancy_medical"), "m.candidate_id=t.candidate_id", array("m.*"))
                ->joinLeft(array("add" => "consultancy_add_company"), "m.add_company_id=add.add_company_id", array("add.name as company_name", "add.id_number as id_number"))
                ->where("t.status='P'");
    ...
    public function getStampingApproved() {
        $db = Zend_Db_Table::getDefaultAdapter();
        $select = $db->select();
        $select->from(array("t" => "consultancy_medical_test"))
                ->joinLeft(array("c" => "consultancy_candidate"), "t.candidate_id=c.candidate_id", array("c.candidate_id", "c.name", "c.passport_no"))
    ...
    public function getMedicallyApprovedById($id) {
        $db = Zend_Db_Table::getDefaultAdapter();
        $select = $db->select();
        $select->from(array("t" => "consultancy_medical_test"))
                ->joinLeft(array("c" => "consultancy_candidate"), "t.candidate_id=c.candidate_id", array("c.candidate_id", "c.name", "c.passport_no"))
                ->joinLeft(array("m" => "consultancy_medical"), "m.candidate_id=t.candidate_id", array("m.*"))
                ->joinLeft(array("add" => "consultancy_add_company"), "m.add_company_id=add.add_company_id", array("add.name as company_name", "add.id_number as id_number"))
                ->where("t.status='P' AND t.candidate_id=$id");
        $results = $db->fetchAll($select);
        $val ='';
        if($results){
            $result = $results[0];
            $val = $result['candidate_id'] . "::" . $result['passport_no'];
        }
        return $val;
    }
 public function getStampingApprovedById($id) {
        $db = Zend_Db_Table::getDefaultAdapter();
        $select = $db->select();
        $select->from(array("t" => "consultancy_medical_test"))
                ->joinLeft(array("c" => "consultancy_candidate"), "t.candidate_id=c.candidate_id", array("c.candidate_id", "c.name", "c.passport_no"))
                ->joinLeft(array("m" => "consultancy_medical"), "m.candidate_id=t.candidate_id", array("m.*"))
                ->joinLeft(array("add" => "consultancy_add_company"), "m.add_company_id=add.add_company_id", array("add.name as company_name", "add.id_number as id_number"))
                ->where("t.status='STP'AND t.candidate_id=$id");
        $results = $db->fetchAll($select);
        $val ='';
        if($results){
            $result = $results[0];
